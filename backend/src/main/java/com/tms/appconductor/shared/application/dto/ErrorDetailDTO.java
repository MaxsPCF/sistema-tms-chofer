...
public record ErrorDetailDTO(
        ...
        @Schema(description = "Código de error único") String errorCode,

        @Schema(description = "Mensaje de error") String message,

        @Schema(description = "Detalles adicionales del error") List<String> details,

        @Schema(description = "Ruta donde ocurrió el error") String path,

        @Schema(description = "Timestamp del error") @JsonFormat(pattern = "yyyy-MM-dd'T'HH:mm:ss.SSS'Z'")

        LocalDateTime timestamp,

        @Schema(description = "ID de trazabilidad") String traceId) {
