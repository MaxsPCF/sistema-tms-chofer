...
import com.tms.appconductor.security.infrastructure.adapter.out.persistence.entity.ModuloEntity;
    ...
    public Flux<Modulo> findMenusByUsuarioIdAndAplicacionCodigo(UUID idUsuario, String codigoAplicacion) {
        ...
        return databaseClient.sql(sql)
                ...
                .map((row, metadata) -> ModuloEntity.builder()
                        ...
                        .puedeCrear(
                                row.get("PuedeCrear", Boolean.class) != null && row.get("PuedeCrear", Boolean.class))
                        .puedeEditar(
                                row.get("PuedeEditar", Boolean.class) != null && row.get("PuedeEditar", Boolean.class))
                        .puedeEliminar(row.get("PuedeEliminar", Boolean.class) != null
                                && row.get("PuedeEliminar", Boolean.class))
                        .puedeAprobar(row.get("PuedeAprobar", Boolean.class) != null
                                && row.get("PuedeAprobar", Boolean.class))
                        .puedeExportar(row.get("PuedeExportar", Boolean.class) != null
                                && row.get("PuedeExportar", Boolean.class))
                        .build())
                .all()
                .map(ModuloEntity::toDomain);
    }
