package com.tms.appconductor.security.infrastructure.adapter.in.rest;
...
import com.tms.appconductor.security.application.port.in.AuthenticateUserUseCase;
import com.tms.appconductor.security.infrastructure.adapter.in.rest.dto.LoginRequest;
...
public class AuthenticateController {
        ...
        @Operation(summary = "Iniciar sesión", description = "Autentica un usuario y retorna JWT + menú dinámico basado en roles")
        @ApiResponses(value = {
        ...
        public Mono<ResponseWrapper<AuthResponse>> login(
                ...
                return authenticateUserUseCase.authenticate(command)
                                .map(authResponse -> ResponseWrapper.success(
                                                authResponse,
                                                "Login exitoso."))
                                .doOnSuccess(response -> log.info("Login exitoso para: {}", request.email()))
                                .doOnError(error -> log.warn("Login fallido para: {} - {}",
